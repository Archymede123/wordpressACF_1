<?php
$attr = imtst_default_shortcode_values($attr);
    ...
    if(isset($last_name) && $last_name==true) $the_posts = imtst_reorder_by_last_name_vc($the_posts, $attr['order']); 
...
if(count($the_posts)>0){
    ...
    /**************************** INCLUDE THEME FILE ********************************/
    $included = false;
    $theme_file = IMTST_DIR_PATH_VC .'themes/'. $attr['theme'] . "/index.php";
    if (file_exists($theme_file)){
    	include $theme_file;
    	$included = true;
    	$final_str .= '<link rel="stylesheet" href="' . IMTST_DIR_URL_VC . 'themes/'.$attr['theme'].'/style.css" type="text/css" media="all">';
    } else {		
		...
		foreach ($plugins_arr as $name){
			$dir = str_replace('indeed-my-testimonials-vc', $name, IMTST_DIR_PATH_VC );
			$style_url = str_replace('indeed-my-testimonials-vc', $name, IMTST_DIR_URL_VC);
    ...
    if($attr['color_scheme']!=''){
        $url_path = IMTST_DIR_URL_VC.'layouts';
    ...
    //RESPONSIVE
    $responsive_css = '';
	$general_settings_data = imtst_general_settings_meta_vc();
    ...
    if(isset($general_settings_data['imtst_custom_css']) && $general_settings_data['imtst_custom_css']!=''){
    	$css .= stripslashes($general_settings_data['imtst_custom_css']);
    }    
   ...
   if(isset($attr['color_scheme']) && $attr['color_scheme']!=''){
  	$final_str .= '<style>
					 .style_'.$attr['color_scheme'].' .ictst_wrapp .owl-tstvc-theme .owl-tstvc-dots .owl-tstvc-dot.active span, .style_'.$attr['color_scheme'].'  .owl-tstvc-theme .owl-tstvc-dots .owl-tstvc-dot:hover span { background: #'.$attr['color_scheme'].' !important; }
					 .style_'.$attr['color_scheme'].' .pag-theme1 .ictst_wrapp .owl-tstvc-theme .owl-tstvc-nav [class*="owl-tstvc-"]:hover{ background-color: #'.$attr['color_scheme'].'; }
					 .style_'.$attr['color_scheme'].' .pag-theme2 .ictst_wrapp .owl-tstvc-theme .owl-tstvc-nav [class*="owl-tstvc-"]:hover{ color: #'.$attr['color_scheme'].'; }
					 .style_'.$attr['color_scheme'].' .pag-theme3 .ictst_wrapp .owl-tstvc-theme .owl-tstvc-nav [class*="owl-tstvc-"]:hover{ background-color: #'.$attr['color_scheme'].';}';
  	$final_str .= '</style>'; 	
  }
...
if(isset($attr['filter_set']) && $attr['filter_set']==1){
            ...
            if($attr['filter_select_t']=='dropdown'){
                //DROPDOWN
                    ...
                    foreach($terms_arr as $term){
                        $team_name = get_term_by('slug', $term, IMTST_TAXONOMY_VC);
  ...
  $final_str .= '</div>';
	...
	foreach($the_posts as $post){
		$terms_arr = get_the_terms( $post->ID, IMTST_TAXONOMY_VC );
        $term_slug_str = '';
        if(isset($terms_arr) && count($terms_arr)>0 && $terms_arr !== FALSE){
        ...
        if(in_array('quote', $show_arr)){
            ...
            $quote = imtst_format_quote_vc( $quote );
        ...
        if(in_array('stars', $show_arr)){
            ...
            $stars = imtst_return_stars_vc($rating);
			if(isset($general_settings_data['imtst_rich_snippets']) && $general_settings_data['imtst_rich_snippets']==1){
        ...
        if(isset($general_settings_data['imtst_rich_snippets']) && $general_settings_data['imtst_rich_snippets']==1){
	        ...
	        $final_str .= imtst_return_google_rich_snippets_vc($rich_arr);
        }
    ...
    $final_str .= "<div class='clear'></div></ul>";#plus one </div>
  ...
  ////WITHOUT FILTER
    ...
    foreach($the_posts as $post){
			...
			if(isset($attr['max_num_desc'])){ 
            	if(strlen($quote)>$attr['max_num_desc']){
					...
					if(isset($attr['read_more']) && $attr['read_more']==1){
						$full_quote = imtst_format_quote_vc( $post->post_content );
						$all_divs = imtst_return_read_more_cq_vc($quote_base_id.'_'.$j, $quote, $full_quote, $attr['color_scheme']);
      ...
      if( $j % $items_per_slide==0 || $j==$total_items ){
      	  $breaker_div++;
      	  $new_div = 1;
          $final_str .= "<div class='clear'></div></ul>";#plus one </div> rich snippet old version
      }else $new_div = 0;
      $j++;
