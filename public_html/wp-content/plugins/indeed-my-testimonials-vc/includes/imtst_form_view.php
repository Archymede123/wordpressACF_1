...
if(isset($_REQUEST['imtst_button'])){
    $client_name = imtst_returnCheckString_vc(@$_REQUEST['imtst_client_name']);
    $client_job = imtst_returnCheckString_vc(@$_REQUEST['imtst_client_job']);
    $quote = imtst_returnCheckString_vc(@$_REQUEST['imtst_quote']);
    $client_url = imtst_returnCheckString_vc(@$_REQUEST['imtst_client_url']);
    $company = imtst_returnCheckString_vc(@$_REQUEST['imtst_company']);
    $company_url = imtst_returnCheckString_vc(@$_REQUEST['imtst_companyurl']);
    $rating = imtst_returnCheckString_vc(@$_REQUEST['imtst_stars']);
    $err_set = FALSE;
    ...
    if(isset($_REQUEST['imtst_req']) && $_REQUEST['imtst_req']!=''){
        ...
        if( imtst_ckreqpostval_vc( $_REQUEST['imtst_req'], 'client_name', $client_name ) ) $err_set = TRUE;
        if( imtst_ckreqpostval_vc( $_REQUEST['imtst_req'], 'client_job', $client_job ) ) $err_set = TRUE;
        if( imtst_ckreqpostval_vc( $_REQUEST['imtst_req'], 'quote', $quote ) ) $err_set = TRUE;
        if( imtst_ckreqpostval_vc( $_REQUEST['imtst_req'], 'client_url', $client_url ) ) $err_set = TRUE;
        if( imtst_ckreqpostval_vc( $_REQUEST['imtst_req'], 'company', $company ) ) $err_set = TRUE;
        if( imtst_ckreqpostval_vc( $_REQUEST['imtst_req'], 'company_url', $company_url ) ) $err_set = TRUE;
        if( imtst_ckreqpostval_vc( $_REQUEST['imtst_req'], 'rating', $rating ) ) $err_set = TRUE;
    }
    //CAPCHA
    if(isset($_REQUEST['imtst_cp_ar_k']) && $_REQUEST['imtst_cp_ar_k']!=''){
    	$capcha = imtst_returnCheckString_vc(@$_REQUEST['imtst_capcha']);
    	if($capcha!=imtst_return_capcha_answer_vc( $_REQUEST['imtst_cp_ar_k'] )) $err_set = TRUE;	
    }
    ...
    if(!$err_set){
        ...
        if (isset($i_post_id) && $i_post_id){
        	...
        	if (!empty($_REQUEST['d_group'])){
        		wp_set_object_terms($i_post_id, $_REQUEST['d_group'], IMTST_TAXONOMY_VC);
        	}
...
$attr = imtst_default_shortcode_values_form($attr);

if(isset($attr['show']) && $attr['show']!='') $show_arr = explode(',', $attr['show']);
$str = "";
    if(in_array('client_name', $show_arr) && isset($attr['client_name'])){
        $req_sign = imtst_check_req_vc(@$attr['req'], 'client_name');
        $value = '';
    ...
    if(in_array('client_image', $show_arr) && isset($attr['client_image'])){
        $req_sign = imtst_check_req_vc(@$attr['req'], 'client_image');
        $value = '';
    ...
    if(in_array('client_job', $show_arr) && isset($attr['client_job'])){
        $req_sign = imtst_check_req_vc(@$attr['req'], 'client_job');
        $value = '';
    ...
    if(in_array('client_url', $show_arr) && isset($attr['client_url'])){
        $req_sign = imtst_check_req_vc(@$attr['req'], 'client_url');
        $value = '';
    ...
    if(in_array('company', $show_arr) && isset($attr['company'])){
        $req_sign = imtst_check_req_vc(@$attr['req'], 'company');
        $value = '';
    ...
    if(in_array('company_url', $show_arr) && isset($attr['company_url'])){
        $req_sign = imtst_check_req_vc(@$attr['req'], 'company_url');
        $value = '';
    ...
    if(in_array('quote', $show_arr) && isset($attr['quote'])){
        $req_sign = imtst_check_req_vc(@$attr['req'], 'quote');
        $value = '';
    ...
    if(in_array('rating', $show_arr) && isset($attr['rating'])){
        $req_sign = imtst_check_req_vc(@$attr['req'], 'rating');
        $value = 0;
    ...
    if(in_array('capcha', $show_arr) && isset($attr['capcha_label'])){
    	$capcha_rand_key = rand(0,5);
    	$str .= "<fieldset class='imtst_fieldset'><label class='imtst_label'>" . $attr['capcha_label'] . "</label>".
      			"<input type='text' name='imtst_capcha' class='imtst_input capcha_field' placeholder='".__('What is the result for ', 'imtst').imtst_return_capcha_question_vc( $capcha_rand_key )."?'>" . 
    			"<input type='hidden' name='imtst_cp_ar_k' value='".$capcha_rand_key."' />".
    			"</fieldset>";
    }
    
    //print the destination group
    if (!empty($attr['d_group']) && $attr['d_group']!=-1){
