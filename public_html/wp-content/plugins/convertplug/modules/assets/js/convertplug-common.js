(function($) {
    ...
    var ConvertPlus = {
        ...
        init: function( e, element, event ) {
            ...
            var body               = $('body');
                ...
                modal              = $('.'+class_id),
                delay              = element.data("onload-delay"),
                delay              = delay * 1000,  // convert delay time from seconds to miliseconds
                load_on_refresh    = element.data('load-on-refresh'),
                scrollTill         = element.data("onscroll-value"),
                display            = false,
                scheduled          = ConvertPlus._isScheduled(),
                nounce             = element.find(".cp-impress-nonce").val(),
                hide_on_device     = element.data('hide-on-devices'),
        ...
        _CpmouseleaveEvent: function(e){
            var md = module,
                ...
                invoke = false;
                display = false;
            if( typeof md !=='undefined' && ConvertPlus._canCpShow() ) {
                if( exit == 'enabled' && e.clientY <= 0 ){
                    display = ( ConvertPlus._isOtherPopupOpen(type) );
         ...
         */
        _canCpShow: function(){
            hide_from_device = ConvertPlus._hideOnDevice(hide_on_device);
            if( hide_from_device ) {
                display = false;
            }
            cookie     = ConvertPlus._getCookie(cookieName);
           ...
           return ( !cookie && scheduled && referred &&!hide_from_device && is_open);
        },
         ...
         * @return {[type]}         [description]
         */
        _hideOnDevice: function( devices ){
         ...
         * @return {[type]} [description]
         */
        _count_inline_impressions : function(){
            var type = module_type,
            main_class = '';
                    ...
                    if( animate_push_page == 1 )  {
                        if( cp_top_offset_container == '' ) {
                            ...
                            jQuery("body").stop().animate({'marginTop':push_margin+'px'}, 900 );
                             ...
                             //console.log("2");
                            jQuery("#cp-top-offset-container").stop().animate({'marginTop': push_margin+'px'}, 900 );
                        }
                    } else {
                        if( cp_top_offset_container == '' ) {
                            jQuery("body").css( 'margin-top', push_margin+'px' );
                        } else {
                            jQuery("#cp-top-offset-container").css( 'margin-top', push_margin+'px' );
                        }
                    }
                }
            }
        ...
        _cal_top_margin_push_down: function( info_bar,animate_push_page, toggle ){
            ...
            if( cp_top_offset_container == '' && rs_flag <= 1 ) {   
                ...
                if( jQuery("#cp-top-offset-container").length > 0 ) {
                    var site_offset = jQuery("#cp-top-offset-container").offset().top;
                    var offset_def_settings = {
                            margin_top: jQuery("#cp-top-offset-container").css('margin-top'),
                            top:  jQuery("#cp-top-offset-container").css('top'),
                    };
                }
            }
    ...
    $( window ).on( 'load', function() {
        ...
        //click event for open module on cusotm class
        jQuery.each(custom_class_arr, function(index,value){
        	if( '' != value && 'undefined' != value && null != value  ) {
	            jQuery("."+value).click(function(event){
    	            ...
    	            if( type == 'modal'){
                        ...
                        var target      = jQuery(".cp-info-bar."+value),
    ...
    $( document ).on( 'mouseleave', function( event ) {
        $(".cp-global-load").each( function(t) {
          	ConvertPlus.init( event, $( this ), 'mouseleave' );
        });
    } );
    ...
    $( document ).scroll( function(event) {
        $(".cp-global-load").each( function(t) {
            ConvertPlus.init( event, $( this ), 'scroll' );
        });
    } );
    ...
    jQuery(document).ready(function(){
       ...
       // ConvertPlus._count_inline_impressions();

       /* setTimeout( function(){  // Initialize tool tip
